...
- (void)addHeadersToRequest {
	...
	if (_params != nil) {
		...
		if ([_params respondsToSelector:@selector(HTTPHeaderValueForContentType)]) {
            if([_params respondsToSelector:@selector(HTTPHeaderValueForContentLength)] && [_params HTTPHeaderValueForContentLength] > 0) {
...
- (void)prepareURLRequest {
    NSLog(@"RKRequest prepareURLRequest START %@", self);
...
>>>>>>> 40eb842dcce0dba0d76063eb9147c89033049f67
    ...
    NSLog(@"RKRequest prepareURLRequest END %@", self);
    [body release];
...
<<<<<<< HEAD
    //NSLog(@"accessing self.URL %@", self.URL);
    //NSLog(@"accessing _method %d", _method);
    //NSLog(@"accessing [_URLRequest HTTPBody] %@", [_URLRequest HTTPBody]);
    NSString* compositCacheKey = [NSString stringWithFormat:@"%@-%d-%@", self.URL, _method, [_URLRequest HTTPBody]];
    //NSLog(@"returning compositCacheKey");
=======
