...
// ML oAuth -- start
@synthesize forceOAuth = _forceOAuth;
...
- (void)addHeadersToRequest {
    ...
    if (_params != nil) {
        //NSLog(@"RKRequest addHeadersToRequest params are: %@ %@", [_params class], _params);
        /*
        // ML temporary logging
        if([_params respondsToSelector:@selector(HTTPHeaderValueForContentType)])
            NSLog(@">>>>>>>>>> _params.HTTPHeaderValueForContentType: %@", [_params performSelector:@selector(HTTPHeaderValueForContentType)]);

        if([_params respondsToSelector:@selector(HTTPHeaderValueForContentLength)])
            NSLog(@">>>>>>>>>> _params.HTTPHeaderValueForContentLength: %d", (int)[_params performSelector:@selector(HTTPHeaderValueForContentLength)]);
        
        if([_params respondsToSelector:@selector(ContentTypeHTTPHeader)])
            NSLog(@">>>>>>>>>> _params.ContentTypeHTTPHeader: %@", [_params performSelector:@selector(ContentTypeHTTPHeader)]);
        */
        // Temporarily support older RKRequestSerializable implementations
        if ([_params respondsToSelector:@selector(HTTPHeaderValueForContentType)]) {
            // ML oAuth -- start
            if(([_params respondsToSelector:@selector(count)] && [_params performSelector:@selector(count)] == 0) ||
               ([_params respondsToSelector:@selector(HTTPHeaderValueForContentLength)] && [_params HTTPHeaderValueForContentLength] == 0))
            {
                //NSLog(@"1 Setting Content-Type header to: %@", @"application/x-www-form-urlencoded");
                [_URLRequest setValue:@"application/x-www-form-urlencoded" forHTTPHeaderField:@"Content-Type"];                
            }
            else
            {
                //NSLog(@"1 Setting Content-Type header to: %@", [_params HTTPHeaderValueForContentType]);
            ...
            //NSLog(@"2 Setting Content-Type header to: %@", [_params performSelector:@selector(ContentTypeHTTPHeader)]);
...
- (BOOL)prepareURLRequest {
    //NSLog(@"RKRequest.prepareURLRequest START");
	[_URLRequest setHTTPMethod:[self HTTPMethod]];

    // ML oAuth -- start
    if(self.forceOAuth)
    {
        //NSLog(@"RKRequest.prepareURLRequest Applying oAuth, method is: %@ URL is %@", [self HTTPMethod], [_URL absoluteString]);
        //NSURL *tmpURL = [_URL copy];
        ...
        if([self.params respondsToSelector:@selector(attachments)])
        {
            ...
            NSMutableDictionary *nonOauthParams = [[NSMutableDictionary alloc] init];
            for(RKParamsAttachment *attachment in requestParams.attachments)
            {
                //NSLog(@"%@ => %@", attachment.name, attachment.fileName);
                if(![attachment.name isEqualToString:@"image"])
                    [nonOauthParams setValue:attachment.fileName forKey:attachment.name];
            }
            [oauth addParameters:nonOauthParams];
            [nonOauthParams release];
        } 
        else if([self.params isKindOfClass:[NSDictionary class]])
        {
            //NSLog(@"Adding attachments directly as a dictionary: %@", self.params);
            [oauth addParameters:(NSDictionary *)self.params];
        }
        else
        {
            //NSLog(@"couldn't add existing params, or no existing params present: %@ %@", [self.params class], self.params);
        }
        NSURLRequest *tmpRequest = [oauth request];
        [_URLRequest setValue:[tmpRequest valueForHTTPHeaderField:@"Authorization"] forHTTPHeaderField:@"Authorization"];
