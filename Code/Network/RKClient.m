	...
	while ([scanner isAtEnd] == NO) {
		...
		if (startsWithParentheses || [scanner scanUpToString:@"(" intoString:nil]) {
			...
			if ([scanner scanUpToString:@")" intoString:&keyPath]) {
                ...
                NSObject *valueForKey = [object valueForKeyPath:keyPath];
                NSString* propertyStringValue;
                if([valueForKey respondsToSelector:@selector(intValue)])
                    propertyStringValue = [NSString stringWithFormat:@"%d", [[object valueForKeyPath:keyPath] intValue]];
                else
                    propertyStringValue = [NSString stringWithFormat:@"%@", [object valueForKeyPath:keyPath]];
...
+ (RKClient*)clientWithBaseURL:(NSString*)baseURL {
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        [client setValue:@"Penzu/1.2.6-iPad CFNetwork/548.0.3 Darwin/10.8.0" forHTTPHeaderField:@"User-Agent"];
	return client;
}
...
- (void)setupRequest:(RKRequest*)request {
    ...
    if(self.forceOAuth)
    {
        // enabling oAuth for the request automatically disables Basic Auth if it was previously set
