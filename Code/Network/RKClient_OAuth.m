...
- (void)setupRequest:(RKRequest*)request {
    NSLog(@"RKClient.setupRequest START %@", self);
    ...
    if(self.forceOAuth)
    {
        NSLog(@"oAuth enabled, setting it for request");
    ...
    NSLog(@"RKClient.setupRequest END");
}
...
<<<<<<< HEAD
    NSLog(@"RKClient.load START");
	RKRequest* request = [[RKRequest alloc] initWithURL:[self URLForResourcePath:resourcePath] delegate:delegate];
	[self setupRequest:request];
	[request autorelease];
	request.method = method;
	request.params = params;
    //NSLog(@"About to send request with method %@, params: %@", method, params);
...
>>>>>>> 40eb842dcce0dba0d76063eb9147c89033049f67
	[request send];
    NSLog(@"RKClient.load END %@", request);
	return request;
}
