...
*NSLogger* feature summary:
  ...
  * Online (application running and connected to _LogTraceger_) and offline (saved logs) log viewing
...
Your logs can be saved to a `.NSLoggerdata` file, and reloaded later. When logging to a file, name your log file with extension `.rawLogTracegerdata` so NSLogger can reopen and process it. You can have clients remotely generating raw logger data files, then send them to you so you can investigate post-mortem.
